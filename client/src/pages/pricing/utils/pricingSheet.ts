...
const STYLE = `
  ...
  * { box-sizing: border-box; -webkit-print-color-adjust: exact; print-color-adjust: exact; }
  ...
  .row { display: grid; grid-template-columns: 1fr 250px; gap: 12px; align-items: stretch; margin-bottom: 12px; }
  .row:last-child { margin-bottom: 0; }
  .cell { min-width: 0; }
  .stack { display: flex; flex-direction: column; gap: 12px; height: 100%; }
  .stack.spread { justify-content: space-between; }
  .grow { flex: 1 1 auto; }
  .panel.vcenter { display: flex; flex-direction: column; justify-content: center; }
  .side { display: flex; flex-direction: column; gap: 12px; height: 100%; }
  .side.spread { justify-content: space-between; }
  .fillcol { display: flex; flex-direction: column; justify-content: space-between; height: 100%; gap: 6px; }
...
export const buildPricingSheetHtml = (p: Pricing): string => {
  ...
  const panel = (label: string, color: string, inner: string, cls = '') => `
    <div class="panel${cls ? ` ${cls}` : ''}" style="background:${color}">
  ...
  const row = (leftInner: string, rightInner: string, sideSpread = false) =>
    `<div class="row"><div class="cell">${leftInner}</div><div class="side${sideSpread ? ' spread' : ''}">${rightInner}</div></div>`;

  const descriptionPanel = panel('DESCRIPTION', '#fff', `
    <div class="grid g3">
  ...
  const logPanel = panel('LOG', C.log, `
  ...
  return `<!doctype html>
<html>
<head><meta charset="utf-8" /><title>${esc(p.description || `Pricing ${p.id}`)}</title><style>${STYLE}</style></head>
<body>
  <div class="header">
    <h1>PRICING</h1>
    <div class="meta">
  ...
  ${row(descriptionPanel, currencyPanel)}
  ${row(logPanel, routePanel)}
  ${row(supplierPanel, incotermsPanel)}
  ${row(costBuildUp, indicatorPanel)}
  ${row(costPriceSap, importAndAttribution, true)}
</body>
</html>`;
};
